...
/*
 * writeBits
 * Writes a value into a bit-packed buffer at the given offset.
 * Example:
 *   writeBits(buffer, 0, 7, 42);
 */
static void writeBits(uint8_t *buffer, uint16_t bitOffset, uint8_t bitCount, uint32_t value) {
  for (uint8_t i = 0; i < bitCount; ++i) {
    uint16_t bitIndex = static_cast<uint16_t>(bitOffset + i);
    uint8_t byteIndex = static_cast<uint8_t>(bitIndex >> 3);
    uint8_t bitInByte = static_cast<uint8_t>(bitIndex & 0x7);
...
/*
 * readBits
 * Reads a bit-packed value from the buffer at the given offset.
 * Example:
 *   uint32_t value = readBits(buffer, 0, 7);
 */
